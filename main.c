...
char user[3];         // grava o usuario
int modo_game = 1;    // grava o modo de jogo (ainda não feito)
int score = 0;        // pontos atuais
int score_record = 0; // maior pontação
...
void imprime_point_in_game();
void imprime_username_in_game();
void imprime_record_in_game();
...
int main()
{
  ...
  do
  {
    ...
    // Perguntando nome
    imprime_mensagem("Digite seu nome (máx 3):");
    scanf("%s", &user);

    // Perguntando o modo de jogo
    imprime_mensagem("Ecolha o modo de jogo(1-Fácil 2-Médio):");
    scanf("%d", &modo_game);
    ...
    imprime_maca(maca);
    imprime_cobra(cobra);
    imprime_username_in_game();
    imprime_point_in_game();
    imprime_record_in_game();
...
void imprime_borda(char **matriz)
{
  ...
  for (idx_x = 0; idx_x < dmi_x; idx_x++)
  {
    ...
    set_char_by_cursor(BORDA_ROW, idx_x, 5);
    set_char_by_cursor(BORDA_ROW, 5, DIMENSAO_Y + 10);
  }
...
// Exibe texto da pontuação atual
void imprime_point_in_game()
{
  int pos_x = 1;
  int pos_y = 2;
  ...
  // Mensagem
  set_char_by_cursor(EMPTY_ROW, pos_x, pos_y);
  printf("SCOORE: %d", score);
  set_char_by_cursor(EMPTY_ROW, pos_x, pos_y);
}

// Exibe a pontual máxima
void imprime_record_in_game()
{
  int pos_x = 1;
  int pos_y = 3;
...
void atualiza_cobra(COBRA *cobra)
{
  ...
  for (idx_offset = 0; idx_offset < OFFSET_X; idx_offset++)
  {
    // Define corpo onde est� a cabe�a
  ...
  do
  {
    ...
    while (no != NULL)
    {
      ...
      for (idx_i = 0; idx_i < 4; idx_i++)
      {
        ...
        if (idx_i == 0)
          atualiza_posicao(dir, D_DOW);
        if (idx_i == 1)
          atualiza_posicao(dir, D_TOP);
        if (idx_i == 2)
          atualiza_posicao(dir, D_LEF);
        if (idx_i == 3)
          atualiza_posicao(dir, D_RHT);
